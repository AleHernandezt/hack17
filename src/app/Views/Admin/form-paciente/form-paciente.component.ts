...
export default class FormPacienteComponent {
  ...
  private formSchema = z.object({
    ...
    id_card_prefix: z.string().min(1, { message: "El prefijo de la cédula es obligatorio." }),
    id_card: z.string().min(1, { message: "El número de cédula es obligatorio." }),
    phone: z.string().min(1, { message: "El teléfono es obligatorio." }),
    address: z.string().min(1, { message: "La dirección es obligatoria." }),
    gender: z.string().min(1, { message: "El género es obligatorio." }),
    economic_status: z.string().min(1, { message: "El estado económico es obligatorio." }),
    pathology_description: z.string().min(1, { message: "La descripción de la patología es obligatoria." }),
    vulnerability_level: z.string().min(1, { message: "El nivel de vulnerabilidad es obligatorio." }),
  });
  ...
  createPaciente(): void {
    ...
    if (isNaN(birthDate.getTime()) || birthDate > today) {
        this.toastrService.error("La fecha de nacimiento debe ser anterior a la fecha actual.", 'Alerta');
        return;
    }

    if (isNaN(Number(this.form.id_card))) {
      this.toastrService.error("El número de cédula debe ser un número.", 'Alerta');
      return;
    }
